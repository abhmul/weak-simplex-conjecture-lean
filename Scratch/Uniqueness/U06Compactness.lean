...
private lemma qform_regularizedCovariance
    {m : ℕ} (R : Matrix (Fin m) (Fin m) ℝ) (ε : ℝ) (a : Coord m) :
    qform (regularizedCovariance R ε) a =
      (1 - ε) * qform R a + ε * ⟪a, a⟫_ℝ := by
  rw [qform, matrixMul_regularizedCovariance]
...
example {m : ℕ} (s : ℕ → Coord m) (C : ℝ)
    (hs : ∀ n, s n ∈ Metric.closedBall 0 C) :
    ∃ sStar : Coord m, ∃ φ : ℕ → ℕ,
      StrictMono φ ∧ Tendsto (s ∘ φ) atTop (nhds sStar) := by
  obtain ⟨sStar, _hsStar, φ, hφ, ht⟩ :=
    (isCompact_closedBall (0 : Coord m) C).tendsto_subseq hs
  exact ⟨sStar, φ, hφ, ht⟩
