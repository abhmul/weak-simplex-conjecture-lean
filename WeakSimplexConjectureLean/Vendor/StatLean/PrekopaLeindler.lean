...
`StatLean/AsymptoticStatistics/ForMathlib/PrekopaLeindler.lean` (Apache-2.0).

Port changes are limited to seven Lean 4.31 syntax repairs from `zero_le _` to `zero_le`
and namespace isolation. The proof and theorem statements otherwise match upstream.
-/
