...
`StatLean/AsymptoticStatistics/ForMathlib/PiGaussian.lean` (Apache-2.0).

Changes are limited to this attribution block, the local `PiWithDensity` import, and outer
`WeakSimplex.Vendor.StatLean` namespace isolation. The declaration bodies match upstream.
-/
