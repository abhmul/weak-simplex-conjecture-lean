...
/-!
# Strict exponential moments for Gaussian maxima

This module derives strict positive exponential-moment comparison from strict upper-tail comparison
using layer cake on a positive-measure interval of levels.
-/
...
theorem gaussianMax_mgf_lt_regularSimplex
  ...
  · exact (continuous_coordinateMax hm).measurable
  · exact hμ
  · exact integrable_exp_mul_coordinateMax hm R hR.1.1 hR.1.2 μ
  · exact integrable_exp_mul_coordinateMax hm 1 Matrix.PosSemidef.one
      (fun i ↦ by simp) μ
  · exact coordinateMax_tail_le_iid hm R hR
  · exact coordinateMax_tail_lt_iid_of_ne_one hm R hR hRne
...
theorem gaussianMax_mgf_eq_regularSimplex_iff
  ...
  · intro heq
    by_contra hRne
    have hlt := gaussianMax_mgf_lt_regularSimplex hm R hR hRne μ hμ
