...
/-!
# Strict lower-orthant comparison

This module combines singular adaptive witnesses, the shared adaptive reduction, and the strict
product theorem for adaptive half-lines.
-/
...
/-- The lower-orthant probability for independent standard Gaussian coordinates. -/
...
/-- Every finite lower orthant is strictly enlarged away from independent covariance. -/
theorem lowerOrthant_gt_iid_of_ne_one
  ...
  have hproduct :
    ...
    centeredTiltedHalfLine_product_lt_of_ne_one hm R hR hRne w.s
  have hmass :
      normalCDF c ^ m ≤
  ...
  exact (ENNReal.ofReal_lt_ofReal_iff_of_nonneg
    (pow_nonneg (normalCDF_pos c).le m)).2 hreal

/-- Equality at one finite lower-orthant threshold characterizes independent covariance. -/
theorem lowerOrthant_eq_iid_iff
  ...
  · intro heq
    by_contra hRne
    have hlt := lowerOrthant_gt_iid_of_ne_one hm R hR hRne c
    rw [heq] at hlt
